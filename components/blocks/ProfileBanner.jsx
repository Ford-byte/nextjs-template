      ...
      {wallpic ? (
        <div className="w-full h-[300px]">
          <Image
            ...
            height={900}
            alt="Profile Banner"
            className="w-full h-full object-cover pointer"
            onClick={() => setSelectedImage(wallpic)}
          />
          {isLogged && (
            <div className="absolute -translate-y-1/2 right-4 lg:right-12 bg-gray-100 p-2 rounded-full">
              <Close className={`size-6 lg:size-12 rotate-45 pointer`} />
            </div>
          )}
        </div>
      ) : (
      ...
      <div className="absolute flex justify-center w-fit rounded-full translate-y-[100%] lg:translate-y-1/2">
        {profile ? (
          <div className="relative">
            ...
            />
            {isLogged && (
              <div className="absolute bottom-6 lg:bottom-8 right-0 bg-gray-100 p-2 rounded-full">
                <Close className={`size-6 lg:size-12 rotate-45 pointer`} />
              </div>
            )}
          </div>
