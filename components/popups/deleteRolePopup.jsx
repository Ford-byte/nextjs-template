...
export default function DeleteRolePopup({ deleteId, onClose, fetchData }) {
  const [toastData, setToastData] = useState(null);

  const handleSubmit = async () => {
    if (!deleteId) {
      setToastData({ status: "error", message: "Invalid role ID." });
      return;
    }

    try {
      await apiClient.delete(`/api/permissions/grant?id=${deleteId}`);
      setToastData({
        status: "success",
  ...
  return (
    <div className="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md bg-gray-200 z-[2]">
      ...
      <p className="flex-1 dark:text-gray-600">
        This action is irreversible. Once deleted, the role cannot be recovered.
      </p>
      <div className="flex flex-col justify-end gap-3 mt-6 sm:flex-row">
        <button
          className="px-6 py-2 rounded-sm secondary-button"
          onClick={onClose}
        >
          No
        </button>
        <button
          className="px-6 py-2 rounded-sm shadow-sm dark:bg-violet-600 dark:text-gray-50 tertiary-button !bg-blue-500 font-bold"
